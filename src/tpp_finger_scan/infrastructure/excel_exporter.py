...
class ExcelExporter:
        ...
        for employee_index, employee in enumerate(session.import_result.employees):
            ...
            for offset, work_date in enumerate(report_dates):
                ...
                if calculation is not None:
                    ...
                    if override and override.code in {SpecialCode.WFH, SpecialCode.W}:
                        sheet.cell(row_index, 2, "WFH")
                    elif override and override.code in {SpecialCode.TL, SpecialCode.S}:
    ...
    def _build_rule_notes(sheet) -> None:
        ...
        rows = [
            ...
            ("WFH", "Work From Home (Dianggap Hadir HD)", "0,00%"),
            ("TL", "Tugas luar (disertai SPT)", "0,00%"),
            ("I", "Izin", "3,00%"),
            ("S", "Rawat inap (disertai bukti)", "0,00%; ditandai kuning"),
        ]
