...
class DeductionEngineTests(unittest.TestCase):
    ...
    def test_special_codes_override_finger_result(self) -> None:
        ...
        wfh = self.engine.calculate(entry, DayOverride(code=SpecialCode.WFH))
        ...
        self.assertEqual(wfh.deductions.total, Decimal("0.00"))
        self.assertEqual(wfh.status, "WFH (Work From Home)")
        self.assertTrue(wfh.finalizable)
